...
import { RPC_VERSION, DEFAULT_RPC, LIX_NETWORK } from '../consts'
import logger from '../logging'
import { timeout } from '../settings'

const log = logger('rpc')

const versionRegex = /LIX:(\d+\.\d+\.\d+)/
/**
 * @class RPCClient
 * @classdesc
 * RPC Client model to query a LIX node. Contains built-in methods to query using RPC calls.
 * @param {string} net - 'MainNet' or 'TestNet' will query the default RPC address found in consts. You may provide a custom URL.
 * @param {string} version - Version of LIX node. Used to check if RPC methods have been implemented. it will default to DEFAULT_RPC found in CONST
 */
class RPCClient {
  constructor (net, version = RPC_VERSION) {
    if (net === LIX_NETWORK.MAIN) {
      this.net = DEFAULT_RPC.MAIN
    } else if (net === LIX_NETWORK.TEST) {
  ...
  /**
   * Gets the version of the LIX node. This method will never be blocked by version. This method will also update the current Client's version to the one received.
   * @return {Promise<string>}
   */
  getVersion () {
  ...
  /**
   * Checks if the provided address is a valid LIX address.
   * @param {string} addr
   * @return {Promise<boolean>}
   */
