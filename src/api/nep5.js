...
import { getRPCEndpoint, getBalance } from './lixDB'
 ...
 * @param {string} url - URL of the LIX node to query.
 ...
 * @param {string} net - 'MainNet', 'TestNet' or a custom LixDB url.
 * @param {string} scriptHash - Contract scriptHash
 ...
 * @param {number} lixrCost - Amount of lixr to pay for transfer.
 * @param {function} [signingFunction] - Optional external signing function.
 * @return {Promise<Response>} RPC response
 */
export const doTransferToken = (net, scriptHash, fromWif, toAddress, transferAmount, lixrCost = 0, signingFunction = null) => {
  ...
  return Promise.all([rpcEndpointPromise, balancePromise])
    .then((values) => {
      ...
      const intents = [
        { assetId: ASSET_ID.LIXR, value: 0.00000001, scriptHash: fromAddrScriptHash }
      ]
      const invoke = { scriptHash, operation: 'transfer', args: [reverseHex(fromAddrScriptHash), toAddrScriptHash, transferAmount] }
      const unsignedTx = Transaction.createInvocationTx(balances, intents, invoke, lixrCost, { version: 1 })
      if (signingFunction) {
