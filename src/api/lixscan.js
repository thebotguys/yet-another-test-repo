...
export const getAPIEndpoint = net => {
  if (networks[net]) return networks[net].extra.lixscan
  return net
}
...
export const getBalance = (net, address) => {
  ...
  return axios.get(apiEndpoint + '/v1/get_balance/' + address).then(res => {
    ...
    log.info(`Retrieved Balance for ${address} from lixscan ${net}`)
    return bal
  })
}
...
export const getMaxClaimAmount = (net, address) => {
  ...
  return axios.get(apiEndpoint + '/v1/get_unclaimed/' + address).then(res => {
    log.info(
      `Retrieved maximum amount of lixr claimable after spending all LIX for ${address} from lixscan ${net}`
    )
...
export const getTransactionHistory = (net, address) => {
  const apiEndpoint = getAPIEndpoint(net)
  return axios
    .get(apiEndpoint + '/v1/get_address_lix/' + address)
    .then(response => {
      log.info(`Retrieved History for ${address} from lixscan ${net}`)
...
const parseTxHistory = rawTxs => {
  ...
  rawTxs.forEach((tx, ind) => {
    ...
    if (ind !== lastItem) {
      ...
      change = {
        LIX: new Fixed8(currBal.LIX || 0).minus(prevBal.LIX || 0),
        LIXR: new Fixed8(currBal.LIXR || 0).minus(prevBal.LIXR || 0)
      }
    } else {
      let symbol = tx.asset_moved === ASSET_ID.LIX ? 'LIX' : 'LIXR'
