 ...
 * The core functionality are sendAsset, claimLixr and doInvoke.
 ...
 * @param {string} config.net - 'MainNet', 'TestNet' or a lix-wallet-db URL.
...
export const claimLixr = config => {
  return fillUrl(config)
    ...
    .catch(err => {
      ...
      log.error(`claimLixr failed with ${err.message}. Dumping config`, dump)
      throw err
    })
}
 ...
 * @param {number} config.lixr - lixrCost of VM script.
 * @param {bool} [config.sendingFromSmartContract] - Optionally specify that the source address is a smart contract that doesn't correspond to the private key.
 * @return {Promise<object>} Configuration object.
 */
export const doInvoke = config => {
  return fillUrl(config)
    ...
    .catch(err => {
      const dump = {
        net: config.net, address: config.address, intents: config.intents, balance: config.balance, script: config.script, lixr: config.lixr, tx: config.tx
      }
...
export const createTx = (config, txType) => {
  ...
  switch (txType) {
    ...
    case 'invocation':
    case 209:
      checkProperty(config, 'balance', 'lixr', 'script')
      if (!config.intents) config.intents = []
      tx = Transaction.createInvocationTx(config.balance, config.intents, config.script, config.lixr, config.override)
      break
    default:
 ...
 * @param {string} config.url - LIX Node URL.
  ...
  return Query.sendRawTransaction(config.tx)
    ...
    .then(res => {
      ...
      if (res.result === true) {
        ...
        const dump = {
          net: config.net, address: config.address, intents: config.intents, balance: config.balance, claims: config.claims, script: config.script, lixr: config.lixr, tx: config.tx
        }
        log.error(
 ...
 * @param {number} assetAmts.LIX - Amt of LIX to send.
 * @param {number} assetAmts.LIXR - Amt of LIXR to send.
 ...
 * @param {string} config.address - Wallet address
 * @param {object} api - The endpoint API object. eg, lixDB or Neoscan.
...
/**
 * Helper method to get the maximum amount of lixr claimable after spending all LIX.
 * @param {object} config - Configuration object.
 * @param {string} config.net - 'MainNet', 'TestNet'
 * @param {string} config.address - Wallet address
 * @param {object} api - The endpoint APi object. eg, lixDB or Neoscan.
 * @return {Promise<Fixed8>} max claimable LIXR
 */
