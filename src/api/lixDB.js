...
import { ASSET_ID, DEFAULT_RPC } from '../consts'
import { Fixed8, reverseHex } from '../utils'
import { networks, httpsOnly, timeout } from '../settings'
import logger from '../logging'

const log = logger('api')
export const name = 'lixDB'

var cachedRPC = null
/**
 * API Switch for MainNet and TestNet
 * @param {string} net - 'MainNet', 'TestNet', or custom lix-wallet-db URL.
 * @return {string} URL of API endpoint.
 */
export const getAPIEndpoint = net => {
  if (networks[net]) return networks[net].extra.lixDB
  return net
}
/**
 * Get balances of LIX and LIXR for an address
...
export const getBalance = (net, address) => {
  ...
  //return rpc.Query.getAccountState(address).execute(DEFAULT_RPC.MAIN.replace(":10331", ":10332").replace("https", "http")).then(res => {
    return axios.get(apiEndpoint + '/v2/address/balance/' + address).then(res => {
      ...
      log.info(`Retrieved Balance for ${address} from lixDB ${net}`)
      return bal
    })
}
 ...
 * @param {string} net - 'MainNet' or 'TestNet'.
 * @param {string} address - Address to check.
 * @return {Promise<Claim>} An object with available and unavailable LIXR amounts.
 */
export const getClaims = (net, address) => {
  ...
  return axios.get(apiEndpoint + '/v2/address/claims/' + address).then(res => {
    ...
    log.info(`Retrieved Claims for ${address} from lixDB ${net}`)
    return new Claims(claimData)
  })
}

/**
 * Gets the maximum amount of lixr claimable after spending all LIX.
 * @param {string} net - 'MainNet' or 'TestNet'.
 * @param {string} address - Address to check.
 * @return {Promise<Fixed8>} An object with available and unavailable LIXR amounts.
 */
export const getMaxClaimAmount = (net, address) => {
  const apiEndpoint = getAPIEndpoint(net)
  return axios.get(apiEndpoint + '/v2/address/claims/' + address).then(res => {
    log.info(
      `Retrieved maximum amount of lixr claimable after spending all LIX for ${address} from lixDB ${net}`
    )
...
export const getTransactionHistory = (net, address) => {
  ...
  return axios
    ...
    .then(response => {
      log.info(`Retrieved History for ${address} from lixDB ${net}`)
      return response.data.history.map(rawTx => {
        return {
          change: {
            LIX: new Fixed8(rawTx.LIX || 0),
            LIXR: new Fixed8(rawTx.LIXR || 0)
          },
...
/**
 * Perform a ClaimTransaction for all available LIXR based on API
 * @param {string} net - 'MainNet' or 'TestNet'.
 ...
 */
export const doClaimAllLixr = (net, privateKey, signingFunction) => {
  log.warn('doClaimAllLixr will be deprecated in favor of claimLixr')
  ...
  return Promise.all([rpcEndpointPromise, claimsPromise])
    .then(values => {
      ...
      if (claims.length === 0) throw new Error('No claimable lixr!')
 ...
 * @param {number} lix - The amount of lix to send to RPX.
 * @param {number} lixrCost - The Lixr to send as SC fee.
 * @return {Promise<Response>} RPC Response
 */
export const doMintTokens = (net, scriptHash, fromWif, lix, lixrCost, signingFunction) => {
  log.warn('doMintTokens will be deprecated in favor of doInvoke')
  const account = new Account(fromWif)
  const intents = [
    { assetId: ASSET_ID.LIX, value: lix, scriptHash: scriptHash }
  ]
  ...
  return Promise.all([rpcEndpointPromise, balancePromise])
    .then(values => {
      ...
      const unsignedTx = Transaction.createInvocationTx(balances, intents, invoke, lixrCost, { attributes })
      if (signingFunction) {
        return signingFunction(unsignedTx, account.publicKey)
      } else {
        return unsignedTx.sign(account.privateKey)
 ...
 * @param {{LIX: number, LIXR: number}} assetAmounts - The amount of each asset (LIX and LIXR) to send, leave empty for 0.
