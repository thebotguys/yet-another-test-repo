...
/**
 * Calculate the inputs required given the intents and lixrCost. lixrCost has to be seperate because it will not be reflected as an TransactionOutput.
 * @param {Balance} balances - Balance of all assets available.
 * @param {TransactionOutput[]} intents - All sending intents
 * @param {number|Fixed8} lixrCost - lixrCost required for the transaction.
 * @param {function} strategy
 * @return {object} {inputs: TransactionInput[], change: TransactionOutput[] }
 */
export const calculateInputs = (balances, intents, lixrCost = 0, strategy = null) => {
  ...
  // Add LIXR cost in
  lixrCost = new Fixed8(lixrCost)
  if (lixrCost.gt(0)) {
    if (requiredAssets[ASSET_ID.LIXR]) {
      requiredAssets[ASSET_ID.LIXR] = requiredAssets[ASSET_ID.LIXR].add(lixrCost)
    } else {
      requiredAssets[ASSET_ID.LIXR] = lixrCost
    }
  }
