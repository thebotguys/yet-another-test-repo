...
const serializeInvocationExclusive = (tx) => {
  ...
  out += tx.script
  if (tx.version >= 1) out += num2fixed8(tx.lixr)
  return out
}

const getInvocationExclusive = (tx) => {
  return { script: tx.script || '', lixr: tx.lixr || 0 }
}
