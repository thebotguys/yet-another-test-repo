...
class Transaction {
   ...
   * @param {number} lixrCost - Lixr to attach for invoking script
   * @param {object} [override={}] - Optional overrides (eg.custom versions)
   * @return {Transaction} Unsigned Transaction
   */
  static createInvocationTx (balances, intents, invoke, lixrCost = 0, override = {}) {
   ...
   * @param {string|Object} assetSymOrTxOut - The symbol of the asset (eg LIX or LIXR) or the TransactionOutput object.
  ...
  /**
   * Calculate the inputs required based on existing outputs provided. Also takes into account the fees required through the lixr property.
   * @param {Balance} balance - Balance to retrieve inputs from.
   * @param {function} strategy
   * @return {Transaction} this
   */
  calculate (balance, strategy = null) {
    const { inputs, change } = core.calculateInputs(balance, this.outputs, this.lixr, strategy)
    this.inputs = inputs
